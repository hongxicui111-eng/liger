...
def get_semantic_ids_no_extra(semantic_ids, codebook_size):
    """
    Build the item -> semantic id mapping using only the RQ-VAE codebooks
    (no extra conflict-resolution sid).

    Unlike ``get_unique_semantic_ids_by_extra_position``, several items are
    allowed to share the exact same (3-codebook) sid. Collisions are left to be
    resolved by the dense retriever (see the hybrid method), which is why the
    extra sid is no longer needed.

    :param semantic_ids: [n_items, 3] array of RQ-VAE codes (values 0..codebook_size-1)
    :return: item_2_semantic_id {item_id(1-based): (sid0, sid1, sid2)},
             max_last_semantic_ids (== codebook_size, the size of the last codebook)
    """
    item_2_semantic_id = {}
    for i in range(len(semantic_ids)):
        id = tuple(int(x) for x in semantic_ids[i])  # length = n_semantic_codebook
        item_2_semantic_id[i + 1] = id
    # the last codebook uses the full codebook_size range (no extra sid appended)
    max_last_semantic_ids = codebook_size
    return item_2_semantic_id, max_last_semantic_ids


def build_prefix2items(item2sid, prefix_depth):
    """
    Build the prefix -> item-set mapping used by the hybrid method.

    :param item2sid: [n_items, n_codebook] array of (expanded) semantic ids,
                     where ``item2sid[i]`` are the sids of item ``i+1``.
    :param prefix_depth: number of leading sids that form a prefix.
    :return: dict mapping ``tuple(prefix)`` -> list of 0-based item indices that
             share that prefix.
    """
    assert prefix_depth <= item2sid.shape[1], (
        f"prefix_depth ({prefix_depth}) must be <= n_codebook ({item2sid.shape[1]})"
    )
    prefix2items = {}
    for i in range(item2sid.shape[0]):
        prefix = tuple(int(x) for x in item2sid[i, :prefix_depth])
        prefix2items.setdefault(prefix, []).append(i)  # 0-based item index
    return prefix2items
...
def load_data(
    ...
    # Decide whether to append the extra (4th) conflict-resolution sid.
    # The original liger adds it; the hybrid method does not and relies on the
    # dense retriever to disambiguate items that share the same prefix.
    if method_config.get("use_extra_sid", True):
        ...
        item_2_semantic_id, max_last_semantic_ids = (
            get_semantic_ids_no_extra(semantic_ids, codebook_size)
        )
        last_codebook_size = codebook_size  # not used for vocab, kept for user_id_offset
        n_codebook = n_semantic_codebook  # no extra sid
