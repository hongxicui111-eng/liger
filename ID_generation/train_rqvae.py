...
def train(config, device, item_embedding, id_split, id_save_location):
    ...
    if os.path.exists(id_save_location):
        ...
        # Also save the RQ-VAE codebook weights for residual decoder usage
        # TIGER_Residual needs the codebook embedding vectors (not the full model)
        # Each codebook is a [codebook_size, latent_size] matrix — very small
        ...
        codebook_path = os.path.join(rqvae_save_dir, f"codebook_weights_{seed_suffix}.pt")
        torch.save(codebook_weights, codebook_path)
        print(f"✓ Saved codebook weights to {codebook_path}")
