...
# =============================================================================
# Residual Decoder Functions
# =============================================================================


def model_forward_residual(model, batch, device, n_codebook, method_config, skip_forward=False):
    """
    Model forward for TIGER_Residual with residual interleaving.

    Prepares encoder inputs and calls model.forward_residual() for training,
    or prepares input_kwargs for generation.
    """
    if method_config["use_id"] == "sid":
        input_sids = batch["input_sids"].to(device)
        attention_mask_sids = batch["attention_mask_sids"].to(device)
        ...
        # item_id mode — fall back to standard forward (residual only for SID)
        input_ids = batch["input_ids"].to(device)
        attention_mask_ids = batch["attention_mask_ids"].to(device)
        labels_ids = batch["labels_ids"].to(device)
...
def evaluate_residual(
    ...
    """Evaluation for TIGER_Residual model using custom generate_residual."""
    ...
    for batch in tqdm(dataloader):
        ...
        with torch.no_grad():
            _, input_kwargs = model_forward_residual(
                model,
                batch,
                device,
                all_semantic_ids.shape[-1],
        ...
        # outputs from generate_residual: [B * num_return_sequences, n_codebook] (no BOS)
        outputs = outputs.reshape(batch_size, num_return_sequences, n_codebook)
        if outputs.shape[-1] < n_codebook:
            to_pad = n_codebook - outputs.shape[-1]
            pad_tensor = torch.zeros(
                (batch_size, outputs.shape[1], to_pad), device=outputs.device
            )
            outputs = torch.cat([outputs, pad_tensor], dim=-1)

        if predicted_embedding is not None:
            returned_predicted_embedding.extend(predicted_embedding)
