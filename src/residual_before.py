            ...
            if k < num_resid and k + 1 < n_codebook:
                ...
                if self.cumulative_residual_loss_weight > 0:
                    ...
                    if self.cumulative_residual_loss_type == "cosine":
                        ...
                        # Batch-wise cross-entropy loss based on inner product (InfoNCE-style).
                        # Each sample's predicted residual (hidden_resid_latent[i]) should
                        # have the highest inner product with its own target (target_cumul[i]),
                        # treating all other samples in the batch as negatives.
                        # sim_matrix[i, j] = <hidden_resid_latent[i], target_cumul[j]>
